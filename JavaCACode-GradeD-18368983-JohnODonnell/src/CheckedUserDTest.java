...
class CheckedUserDTest {

    //Default ideal value for CheckedUserD class
    private String defaultName = "John Doe", defaultEmail = "john.doe@ucdconnect.ie";
    private int defaultAge = 20;

    //User and CheckedUserD Variables
    private CheckedUserD Cuser, tempCuser;
    private User user;
    ...
    @BeforeEach
    void setUp() {
        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD before the test(Should not throw exception as passed valid data)
        try {
            // Before each test set Cuser back to the default
            Cuser = new CheckedUserD(defaultName, defaultAge, defaultEmail);
        } catch (Exception e){
            fail("Constructor failed to create the CheckedUserD in setUp so test failed and was aborted\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        }
    ...
    @Test
    @DisplayName("Test CheckedUserD is an instance of User Interface (b)")
    void testCheckedUserDInstanceOfUser() {

        //Checking that CheckedUserD object is instance of User
        assertTrue(Cuser instanceof User, "CheckedUserD does not implement User");

    }

    @DisplayName("Test getName Accessor Method in CheckedUserD  (b)")
    @Test
    void testGetName() {
        //assertAll so that all assertions are run and reported together
        assertAll("Testing getName in CheckedUserD",
                // Assertion to test that the getName method works on a CheckedUserD object
                () -> assertEquals(Cuser.getName(), defaultName, "\nThe name returned from getName on CheckedUserD object was not equal to the default name: " + defaultName + "\n"),
                // Assertion to test that the getName method works on a CheckedUserD object using the User type
                () -> assertEquals(user.getName(), defaultName,  "\nThe name returned from getName on CheckedUserD object using User interface was not equal to the default name: " + defaultName + "\n")
        );
    }


    @DisplayName("Test setName Mutator Method in CheckedUserD  (b)")
    @Test
    void testSetName() {
        // Setting a String variable new temp that is being set over name in the CheckedUserD object
        String newTemp = "David";

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD for the test (Should not throw exception as passed valid data)
        try {
            //Create tempCuser to be modified for the test (needs to have email blank inorder to test setName and set email)
            tempCuser = new CheckedUserD("John Doe");
        } catch (InvalidNameException e) {
            fail("Constructor failed to create the CheckedUserD so test failed and was aborted\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        } catch (Exception e){
        ...
        try {
            // Using setName on CheckedUserD object
        ...
        //assertAll so that all assertions are run and reported together
        assertAll("Testing setName on CheckedUserD",
                // Assertion to test that the getName method returns the new set name on a CheckedUserD object
                () -> assertEquals(tempCuser.getName(), newTemp, "\nThe name returned from getName on CheckedUserD object was not equal to the new name: " + newTemp + "\n When setName was used on CheckedUserD object\n"),
                // Assertion to test that the getName method returns the new set name on a CheckedUserD object using the User type
                () -> assertEquals(user.getName(), newTemp,  "\nThe name returned from getName on CheckedUserD object using User interface was not equal to the new name: " + newTemp + "\n When setName was used on CheckedUserD object\n")
        );


        // Setting a String variable new temp 2 that is being set over name in the CheckedUserD object using the User type
        String newTemp2 = "Bob Smith";

        //Try statement to catch possible exceptions thrown by setName (Should not throw exception as passed valid data)
        try{
            // Using setName on CheckedUserD object using the User type
        ...
        //assertAll so that all assertions are run and reported together
        assertAll("Testing setName on CheckedUserD using the User type",
                // Assertion to test that the getName method returns the new set name on a CheckedUserD object
                () -> assertEquals(tempCuser.getName(), newTemp2, "\nThe name returned from getName on CheckedUserD object was not equal to the new name: " + newTemp2 + "\n When setName was used on CheckedUserD object\n"),
                // Assertion to test that the getName method returns the new set name on a CheckedUserD object using the User type
                () -> assertEquals(user.getName(), newTemp2,  "\nThe name returned from getName on CheckedUserD object using User interface was not equal to the new name: " + newTemp2 + "\n When setName was used on CheckedUserD object\n")
        );
    }

    @DisplayName("Test getEmail Accessor Method in CheckedUserD (b)")
    @Test
    void testGetEmail() {
        //assertAll so that all assertions are run and reported together
        assertAll("Testing getEmail in CheckedUserD",
                // Assertion to test that the getEmail method works on a CheckedUserD object
                () -> assertEquals(Cuser.getEmail(), defaultEmail, "\nThe name returned from getEmail on CheckedUserD object was not equal to the default email: " + defaultEmail + "\n"),
                // Assertion to test that the getEmail method works on a CheckedUserD object using the User type
                () -> assertEquals(user.getEmail(), defaultEmail,  "\nThe name returned from getEmail on CheckedUserD object using User interface was not equal to the default email: " + defaultEmail + "\n")
        );
    }

    @DisplayName("Test setEmail Mutator Method in CheckedUserD (b)")
    @Test
    void testSetEmail() {
        // Setting a String variable new temp that is being set over email in the CheckedUserD object
        ...
        try {
            // Using setEmail on CheckedUserD object
        ...
        //assertAll so that all assertions are run and reported together
        assertAll("Testing setEmail on CheckedUserD",
                // Assertion to test that the getEmail method returns the new set email on a CheckedUserD object
                () -> assertEquals(tempCuser.getEmail(), newTemp, "\nThe email returned from getEmail on CheckedUserD object was not equal to the new email: " + newTemp + "\n When setEmail was used on CheckedUserD object\n"),
                // Assertion to test that the getEmail method returns the new set email on a CheckedUserD object using the User type
                () -> assertEquals(user.getEmail(), newTemp,  "\nThe email returned from getEmail on CheckedUserD object using User interface was not equal to the new email: " + newTemp + "\n When setEmail was used on CheckedUserD object\n")
        );


        // Setting a String variable new temp 2 that is being set over email in the CheckedUserD object using the User type
        String newTemp2 = "john@ucdconnect.ie";

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD for the test (Should not throw exception as passed valid data)
        try {
            //Create tempCuser to be modified for the tests
            tempCuser = new CheckedUserD("John");
        } catch (InvalidNameException e) {
            fail("Constructor failed to create the CheckedUserD so test failed and was aborted\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        } catch (Exception e){
            fail("Test Failed because constructor throw an unexpected exception\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        ...
        //Try statement to catch possible exceptions thrown by setEmail (Should not throw exception as passed valid data)
        try {
            // Using setEmail on CheckedUserD object using the User type
        ...
        //assertAll so that all assertions are run and reported together
        assertAll("Testing setEmail on CheckedUserD using the User type",
                // Assertion to test that the getEmail method returns the new set email on a CheckedUserD object
                () -> assertEquals(tempCuser.getEmail(), newTemp2, "\nThe email returned from getEmail on CheckedUserD object was not equal to the new email: " + newTemp2 + "\n When setEmail was used on CheckedUserD object\n"),
                // Assertion to test that the getEmail method returns the new set email on a CheckedUserD object using the User type
                () -> assertEquals(user.getEmail(), newTemp2,  "\nThe email returned from getEmail on CheckedUserD object using User interface was not equal to the new email: " + newTemp2 + "\n When setEmail was used on CheckedUserD object\n")
        );
    }

    @DisplayName("Test getAge Accessor Method in CheckedUserD (b)")
    @Test
    void testGetAge() {
        //assertAll so that all assertions are run and reported together
        assertAll("Testing getAge in CheckedUserD",
                // Assertion to test that the getAge method works on a CheckedUserD object
                () -> assertEquals(Cuser.getAge(), defaultAge, "\nThe age returned from getAge on CheckedUserD object was not equal to the default age: " + defaultAge + "\n"),
                // Assertion to test that the getAge method works on a CheckedUserD object using the User type
                () -> assertEquals(user.getAge(), defaultAge,  "\nThe age returned from getAge on CheckedUserD object using User interface was not equal to the default age: " + defaultAge + "\n")
        );
    }

    @DisplayName("Test setAge Mutator Method in CheckedUserD (b)")
    @Test
    void testSetAge() {
        // Setting a String variable new temp that is being set over age in the CheckedUserD object
        int newTemp = 22;

        //Try statement to catch possible exceptions thrown by setAge (Should not throw exception as passed valid data)
        try {
            // Using setAge on CheckedUserD object
        ...
        //assertAll so that all assertions are run and reported together
        assertAll("Testing setAge on CheckedUserD",
                // Assertion to test that the getAge method returns the new set age on a CheckedUserD object
                () -> assertEquals(Cuser.getAge(), newTemp, "\nThe age returned from getAge on CheckedUserD object was not equal to the new age: " + newTemp + "\n When setAge was used on CheckedUserD object\n"),
                // Assertion to test that the getAge method returns the new set age on a CheckedUserD object using the User type
                () -> assertEquals(user.getAge(), newTemp,  "\nThe age returned from getAge on CheckedUserD object using User interface was not equal to the new age: " + newTemp + "\n When setAge was used on CheckedUserD object\n")
        );
        ...
        //assertAll so that all assertions are run and reported together
        assertAll("Testing setAge on CheckedUserD using the User type",
                // Assertion to test that the getAge method returns the new set age on a CheckedUserD object
                () -> assertEquals(Cuser.getAge(), newTemp2, "\nThe age returned from getAge on CheckedUserD object was not equal to the new age: " + newTemp2 + "\n When setAge was used on CheckedUserD object\n"),
                // Assertion to test that the getAge method returns the new set age on a CheckedUserD object using the User type
                () -> assertEquals(user.getAge(), newTemp2,  "\nThe age returned from getAge on CheckedUserD object using User interface was not equal to the new age: " + newTemp2 + "\n When setAge was used on CheckedUserD object\n")
        );
    }
    ...
    void testConstructorName(){
        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            //Create constructorTest using the just name constructor
            CheckedUserD constructorTest = new CheckedUserD(defaultName);
            ...
            fail("Constructor failed to create the CheckedUserD so test failed due to an invalid name\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        } catch (Exception e){
            fail("Test Failed because constructor throw an unexpected exception\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        }
    }
    ...
    void testConstructorNameAge(){
        ...
        try {
            ...
            CheckedUserD constructorTest = new CheckedUserD(defaultName, defaultAge);
            ...
            fail("Constructor failed to create the CheckedUserD so test failed due to an invalid age\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        } catch (Exception e){
            fail("Test Failed because constructor throw an unexpected exception\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        }
    }
    ...
    void testConstructorNameAgeEmail(){
        ...
        try {
            ...
            CheckedUserD constructorTest = new CheckedUserD(defaultName, defaultAge, defaultEmail);
            ...
            fail("Constructor failed to create the CheckedUserD so test failed due to an invalid email\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        } catch (Exception e){
            fail("Test Failed because constructor throw an unexpected exception\n\nException Stack Trace:\n" + exceptionToString(e) + "\n\nFail Trace:\n");
        }
    }
    ...
    void testNameOneWordConstructors(){
        // Setting a String variable oneWordName that is being set in the CheckedUserD
        String oneWordName = "David";

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            //Create constructorTest using the just name constructor with a one word name
            CheckedUserD constructorTest = new CheckedUserD(oneWordName);
        ...
        try {
            ...
            CheckedUserD constructorTest = new CheckedUserD(oneWordName, defaultAge);
        ...
        try {
            ...
            CheckedUserD constructorTest = new CheckedUserD(oneWordName, defaultAge, "david@ucd.ie");
    ...
    void testNameOneWordSetName(){
        // Setting a String variable oneWordName that is being set over name in the CheckedUserD object
        String oneWordName = "David";
    ...
    void testNameTwoWordConstructors(){
        // Setting a String variable twoWordName that is being set in the CheckedUserD
        String twoWordName = "David Smith";

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            //Create constructorTest using the just name constructor with a two word name
            CheckedUserD constructorTest = new CheckedUserD(twoWordName);
        ...
        try {
            ...
            CheckedUserD constructorTest = new CheckedUserD(twoWordName, defaultAge);
        ...
        try {
            ...
            CheckedUserD constructorTest = new CheckedUserD(twoWordName, defaultAge, "david.smith@ucd.ie");
    ...
    void testNameTwoWordSetName(){
        // Setting a String variable twoWordName that is being set over name in the CheckedUserD object
        String twoWordName = "David Smith";
    ...
    void testNameTwoPlusWordConstructors(){
        // Setting a String variable twoPlusWordName that is being set in the CheckedUserD
        ...
        assertAll("Testing Name Validation for Two Plus Word Name in Constructors",
                ...
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(twoPlusWordName), "The name constructor didn't throw InvalidNameException when a name with two plus words was inputted" ),
                ...
                () ->assertThrows(InvalidNameException.class, () -> new CheckedUserD(twoPlusWordName, defaultAge), "The name and age constructor didn't throw InvalidNameException when a name with two plus words was inputted" ),
                ...
                () ->assertThrows(InvalidNameException.class, () -> new CheckedUserD(twoPlusWordName, defaultAge, "david.smith.bob"), "The name, age and email constructor didn't throw InvalidNameException when a name with two plus words was inputted" )
        );
    ...
    void testNameTwoPlusWordSetName(){
        // Setting a String variable twoPlusWordName that is being set over name in the CheckedUserD object
        String twoPlusWordName = "David Smith Bob";
    ...
    void testNamePrefix(){
        ...
        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            //CheckedUserDs for testing all prefixes
            CheckedUserD defaultCuserDrPrefixTest = new CheckedUserD(defaultName), defaultCuserDrTest = new CheckedUserD(defaultName), defaultCuserMrPrefixTest = new CheckedUserD(defaultName), defaultCuserMrTest = new CheckedUserD(defaultName), defaultCuserMrsPrefixTest = new CheckedUserD(defaultName), defaultCuserMrsTest = new CheckedUserD(defaultName), defaultCuserMsPrefixTest = new CheckedUserD(defaultName), defaultCuserMsTest = new CheckedUserD(defaultName), defaultCuserMissPrefixTest = new CheckedUserD(defaultName), defaultCuserMissTest = new CheckedUserD(defaultName), defaultCuserProfPrefixTest = new CheckedUserD(defaultName), defaultCuserProfTest = new CheckedUserD(defaultName);
            ...
            assertAll("Testing Name Can Not Have Prefixes or Name Can Not Be a Prefix",
                    () -> assertAll("Testing Dr Prefix",
                            ...
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameDrPrefix), "The name constructor didn't throw InvalidNameException when inputted with a name with the Dr prefix\nTest name: " + nameDrPrefix),
                            //Assertion that if name and age constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameDrPrefix, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Dr prefix\nTest name: " + nameDrPrefix),
                            //Assertion that if name, age and email constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameDrPrefix, defaultAge, "dr.joe@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Dr prefix\nTest name: " + nameDrPrefix),
                            //Assertion that if name constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameDr), "The name constructor didn't throw InvalidNameException when inputted with a name with the Dr prefix as name\nTest name: " + nameDr),
                            //Assertion that if name and age constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameDr, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Dr prefix as name\nTest name: " + nameDr),
                            //Assertion that if name, age and email constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameDr, defaultAge, "dr@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Dr prefix as name\nTest name: " + nameDr),
                    ...
                    () -> assertAll("Testing Mr Prefix",
                            //Assertion that if name constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrPrefix), "The name constructor didn't throw InvalidNameException when inputted with a name with the Mr prefix\nTest name: " + nameMrPrefix),
                            //Assertion that if name and age constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrPrefix, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Mr prefix\nTest name: " + nameMrPrefix),
                            //Assertion that if name, age and email constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrPrefix, defaultAge, "mr.bob@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Mr prefix\nTest name: " + nameMrPrefix),
                            //Assertion that if name constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMr), "The name constructor didn't throw InvalidNameException when inputted with a name with the Mr prefix as name\nTest name: " + nameMr),
                            //Assertion that if name and age constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMr, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Mr prefix as name\nTest name: " + nameMr),
                            //Assertion that if name, age and email constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMr, defaultAge, "mr@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Mr prefix as name\nTest name: " + nameMr),
                            //Assertion that if setNAme is inputted a name with a prefix it should throw InvalidNameException
                    ...
                    () -> assertAll("Testing Mrs Prefix",
                            //Assertion that if name constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrsPrefix), "The name constructor didn't throw InvalidNameException when inputted with a name with the Mrs prefix\nTest name: " + nameMrsPrefix),
                            //Assertion that if name and age constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrsPrefix, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Mrs prefix\nTest name: " + nameMrsPrefix),
                            //Assertion that if name, age and email constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrsPrefix, defaultAge, "mrs.jane@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Mrs prefix\nTest name: " + nameMrsPrefix),
                            //Assertion that if name constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrs), "The name constructor didn't throw InvalidNameException when inputted with a name with the Mrs prefix as name\nTest name: " + nameMrs),
                            //Assertion that if name and age constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrs, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Mrs prefix as name\nTest name: " + nameMrs),
                            //Assertion that if name, age and email constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMrs, defaultAge, "mrs@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Mrs prefix as name\nTest name: " + nameMrs),
                            //Assertion that if setNAme is inputted a name with a prefix it should throw InvalidNameException
                    ...
                    () -> assertAll("Testing Ms Prefix",
                            //Assertion that if name constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMsPrefix), "The name constructor didn't throw InvalidNameException when inputted with a name with the Ms prefix\nTest name: " + nameMsPrefix),
                            //Assertion that if name and age constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMsPrefix, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Ms prefix\nTest name: " + nameMsPrefix),
                            //Assertion that if name, age and email constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMsPrefix, defaultAge, "ms.smith@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Ms prefix\nTest name: " + nameMsPrefix),
                            //Assertion that if name constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMs), "The name constructor didn't throw InvalidNameException when inputted with a name with the Ms prefix as name\nTest name: " + nameMs),
                            //Assertion that if name and age constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMs, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Ms prefix as name\nTest name: " + nameMs),
                            //Assertion that if name, age and email constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMs, defaultAge, "ms@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Ms prefix as name\nTest name: " + nameMs),
                            //Assertion that if setNAme is inputted a name with a prefix it should throw InvalidNameException
                    ...
                    () -> assertAll("Testing Miss Prefix",
                            //Assertion that if name constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMissPrefix), "The name constructor didn't throw InvalidNameException when inputted with a name with the Miss prefix\nTest name: " + nameMissPrefix),
                            //Assertion that if name and age constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMissPrefix, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Miss prefix\nTest name: " + nameMissPrefix),
                            //Assertion that if name, age and email constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMissPrefix, defaultAge, "miss.lane@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Miss prefix\nTest name: " + nameMissPrefix),
                            //Assertion that if name constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMiss), "The name constructor didn't throw InvalidNameException when inputted with a name with the Miss prefix as name\nTest name: " + nameMiss),
                            //Assertion that if name and age constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMiss, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Miss prefix as name\nTest name: " + nameMiss),
                            //Assertion that if name, age and email constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameMiss, defaultAge, "miss@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Miss prefix as name\nTest name: " + nameMiss),
                            //Assertion that if setNAme is inputted a name with a prefix it should throw InvalidNameException
                    ...
                    () -> assertAll("Testing Prof Prefix",
                            //Assertion that if name constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameProfPrefix), "The name constructor didn't throw InvalidNameException when inputted with a name with the Prof prefix\nTest name: " + nameProfPrefix),
                            //Assertion that if name and age constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameProfPrefix, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Prof prefix\nTest name: " + nameProfPrefix),
                            //Assertion that if name, age and email constructor is inputted a name with a prefix it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameProfPrefix, defaultAge, "prof.simon@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Prof prefix\nTest name: " + nameProfPrefix),
                            //Assertion that if name constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameProf), "The name constructor didn't throw InvalidNameException when inputted with a name with the Prof prefix as name\nTest name: " + nameProf),
                            //Assertion that if name and age constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameProf, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with the Prof prefix as name\nTest name: " + nameProf),
                            //Assertion that if name, age and email constructor is inputted a prefix as name  it should throw InvalidNameException
                            () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameProf, defaultAge, "prof@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with the Prof prefix as name\nTest name: " + nameProf),
                            //Assertion that if setNAme is inputted a name with a prefix it should throw InvalidNameException
    ...
    void testNameNoDigits(){
        //name and CheckedUserD for testing digits in name
        String nameNumber = "J0hn";
        CheckedUserD CuserDigit;

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            CuserDigit = new CheckedUserD(defaultName);
            ...
            assertAll("Testing Name can not Contain Digits",
                    ...
                    () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameNumber), "The name constructor didn't throw InvalidNameException when inputted with a name with a digit\nTest name: " + nameNumber),
                    //Assertion that if name constructor is inputted a name with a digit it should throw InvalidNameException
                    () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameNumber, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with a digit\nTest name: " + nameNumber),
                    //Assertion that if name constructor is inputted a name with a digit it should throw InvalidNameException
                    () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameNumber, defaultAge, "j0hn@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with a digit\nTest name: " + nameNumber),
    ...
    void testNameNoSymbols(){
        //name and CheckedUserD for testing symbols in name
        String nameSymbol = "John!";
        CheckedUserD CuserSymbol;

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            CuserSymbol = new CheckedUserD(defaultName);
            ...
            assertAll("Testing Name can not Contain Symbols",
                    ...
                    () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameSymbol), "The name constructor didn't throw InvalidNameException when inputted with a name with a symbol\nTest name: " + nameSymbol),
                    //Assertion that if name constructor is inputted a name with a symbol it should throw InvalidNameException
                    () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameSymbol, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with a symbol\nTest name: " + nameSymbol),
                    //Assertion that if name constructor is inputted a name with a symbol it should throw InvalidNameException
                    () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameSymbol, defaultAge, "john!@ucd.ie"), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with a symbol\nTest name: " + nameSymbol),
    ...
    void testStartNameCap(){
        ...
        assertAll("Testing Name(s) start with a Capital Letter ",
                ...
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameTest), "The name constructor didn't throw InvalidNameException when inputted with a name with no capitals\nTest name: " + nameTest),
                //Assertion that if name and age constructor is inputted a name with no capitals it should throw InvalidNameException
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameTest, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with no capitals\nTest name: " + nameTest),
                ...
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameTest, defaultAge, defaultEmail), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with no capitals\nTest name: " + nameTest),
    ...
    void testNameWhiteSpace(){
        ...
        assertAll("Testing Name can not have Extra White Space Characters",
                ...
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameTest), "The name constructor didn't throw InvalidNameException when inputted with a name with extra white space\nTest name: " + nameTest),
                //Assertion that if name and age constructor is inputted a name with extra white space should throw InvalidNameException
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameTest, defaultAge), "The name and age constructor didn't throw InvalidNameException when inputted with a name with extra white space\nTest name: " + nameTest),
                ...
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(nameTest, defaultAge, defaultEmail), "The name, age and email constructor didn't throw InvalidNameException when inputted with a name with extra white space\nTest name: " + nameTest),
    ...
    void testSetNameEmailNull(){
        // Setting a String variable nameTest that is being set over name in the CheckedUserD object
        String nameTest = "David";

        //Set name to name test on a CheckedUserD with a set Email
    ...
    void testAgeLessThan18(){
        ...
        assertAll("Testing Age can not be Less Than 18",
                ...
                () -> assertThrows(InvalidAgeException.class, () -> new CheckedUserD(defaultName, ageTest), "The name and age constructor didn't throw InvalidAgeException when inputted with an age less than 18 \nTest age: " + ageTest),
                //Assertion that if name, age and email constructor is inputted an age less than 18  it should throw InvalidAgeException
                () -> assertThrows(InvalidAgeException.class, () -> new CheckedUserD(defaultName, ageTest, defaultEmail), "The name, age and email constructor didn't throw InvalidAgeException when inputted with an age less than 18 \nTest age: " + ageTest),
    ...
    void testAgeGreaterThan30(){
        ...
        assertAll("Testing Age can not be Greater Than 30",
                ...
                () -> assertThrows(InvalidAgeException.class, () -> new CheckedUserD(defaultName, ageTest), "The name and age constructor didn't throw InvalidAgeException when inputted with an age greater than 30 \nTest age: " + ageTest),
                //Assertion that if name, age and email constructor is inputted an age greater than 30  it should throw InvalidAgeException
                () -> assertThrows(InvalidAgeException.class, () -> new CheckedUserD(defaultName, ageTest, defaultEmail), "The name, age and email constructor didn't throw InvalidAgeException when inputted with an age greater than 30 \nTest age: " + ageTest),
    ...
    void testEmailOneWordName(){
        ...
        try {
            //Create constructorTest using the name age and email constructor with a one word name
            CheckedUserD constructorTest = new CheckedUserD(nameTest, defaultAge, emailTest);
        ...
        try {
            //Create constructorTest using the just name constructor
            CheckedUserD constructorTest = new CheckedUserD(nameTest);
    ...
    void testEmailNoDigits(){
        //CheckedUserD, Name and Email for testing email can't contain digits
        String nameNumber = "John", emailNumber = "j0hn@ucd.ie";
        CheckedUserD CuserDigit;

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            CuserDigit = new CheckedUserD(nameNumber);
            ...
            assertAll("Testing Email can not Contain Digits",
                    ...
                    () -> assertThrows(InvalidEmailException.class, () -> new CheckedUserD(nameNumber, defaultAge, emailNumber), "The name, age and email constructor didn't throw InvalidEmailException when inputted with a email with a digit\nTest email: " + emailNumber),
    ...
    void testEmailNoSymbols(){
        //CheckedUserD, Name and Email for testing email cant contain symbols
        String nameSymbol = "John", emailSymbol = "john!.@ucd.ie";
        CheckedUserD CuserSymbol;

        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            CuserSymbol = new CheckedUserD(nameSymbol);
            ...
            assertAll("Testing Email can not Contain Symbols ",
                    ...
                    () -> assertThrows(InvalidEmailException.class, () -> new CheckedUserD(nameSymbol, defaultAge, emailSymbol), "The name, age and email constructor didn't throw InvalidEmailException when inputted with an email with a symbol\nTest email: " + emailSymbol),
    ...
    void testEmailCapital(){
        ...
        try {
            ...
            CheckedUserD constructorTest = new CheckedUserD(defaultName, defaultAge, emailTest);
    ...
    void testEmailMatchName(){
        ...
        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            tempCuser = new CheckedUserD(defaultName);
            ...
            assertAll("Testing Email Matches the Name ",
                    ...
                    () -> assertThrows(InvalidEmailException.class, () -> new CheckedUserD(defaultName, defaultAge, testEmail), "The name, age and email constructor didn't throw InvalidEmailException when inputted with an email that does not match name\nTest email: " + testEmail),
    ...
    void testOrderExceptions(){
        ...
        assertAll("Testing the order of Exceptions",
                ...
                () -> assertThrows(InvalidNameException.class, () -> new CheckedUserD(invalidName, invalidAge, invalidEmail), "Name was not checked first so InvalidNameException was not thrown\n"),
                //Assertion that age should be checked before email so InvalidAgeException should be thrown
                () -> assertThrows(InvalidAgeException.class, () -> new CheckedUserD(defaultName, invalidAge, invalidEmail), "Age was not checked before email so InvalidAgeException was not thrown\n")
        );
    }
    ...
    void testToStringName(){
        ...
        try {
            ...
            assertEquals("CheckedUserD: John Doe", tempCuser.toString(), "Failed because the string outputted by toString did not match the expected string: \"CheckedUserD: John Doe\"");
    ...
    void testToStringNameAge(){
        //Try statement to catch if the constructor throws an exception in creating the CheckedUserD(Should not throw exception as passed valid data)
        try {
            tempCuser = new CheckedUserD(defaultName, defaultAge);

            //Assertion to check that toString for name and age outputs the correct string
            assertEquals("CheckedUserD: John Doe (20)", tempCuser.toString(), "Failed because the string outputted by toString did not match the expected string: \"CheckedUserD: John Doe (20)\"");
    ...
    void testToStringNameAgeEmail(){
        ...
        assertEquals("CheckedUserD: John Doe (20), john.doe@ucdconnect.ie", Cuser.toString(), "Failed because the string outputted by toString did not match the expected string: \"CheckedUserD: John Doe (20), john.doe@ucdconnect.ie\"");
    }
    ...
    void testToStringNameEmail(){
        ...
        try {
            ...
            //Assertion to check that toString for name and age outputs the correct string
            assertEquals("CheckedUserD: John Doe, john.doe@ucdconnect.ie", tempCuser.toString(), "Failed because the string outputted by toString did not match the expected string: \"CheckedUserD: John Doe, john.doe@ucdconnect.ie\"");
